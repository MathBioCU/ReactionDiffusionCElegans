# Helper functions used to simulate and analyze RD model.

#Simulate reactions in system
system <- function(t,y,parameters){
  #Parameters
  a = parameters[1]
  b = parameters[2]
  c = parameters[3]
...
dslnex = function(x) {
  ...
  y[1] = 1 - a*p^r/(h+p^r) - x[1]
  y[2] = 1 - b*p^r/(h+p^r) - c*x[2]
  y #Return y
}

#Calculates the derivate of the reaction fucntions at (u0,v0)
findDerivatives = function(a,b,c,h,Km,Ki,r,u0,v0) {
  Z = v0^r+h*(Km*(1+u0/Ki) + v0)^r
  fu = r*Km*h*a*v0^r*(Km*(u0/Ki+1)+v0)^(r-1)/Ki/Z^2 - 1
  fv = -a*r*v0^(r-1)/Z + a*v0^r*(r*v0^(r-1)+r*h*(v0+Km*(u0/Ki+1))^(r-1))/Z^2
  gu = r*h*Km*b*v0^r*(Km*(u0/Ki+1)+v0)^(r-1)/Ki/Z^2
  gv = -b*r*v0^(r-1)/Z + b*v0^r*(r*v0^(r-1)+r*h*(v0+Km*(u0/Ki+1))^(r-1))/Z^2 - c
